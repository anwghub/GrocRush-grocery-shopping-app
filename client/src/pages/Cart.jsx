...
const Cart = () => {
    const { products, currency, cartItems, removeFromCart, setCartItems, addToCart, getCartAmount, getCartCount, navigate, updateCartItem, axios, user } = useAppContext();
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => {
                            ...
                            setCartArray(prev => prev.filter(item => item._id !== product._id));
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address, index) => (
                                    <p key={index} onClick={() => { setSelectedAddress(addresses); setShowAddress(false) }} className="text-gray-500 p-2 hover:bg-gray-100">
